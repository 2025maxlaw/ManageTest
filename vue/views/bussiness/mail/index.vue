<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="mailList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="消息类型 =普通消息 2=全站消息" align="center" prop="type" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="操作人" prop="opertorId">
          <el-input v-model="form.opertorId" placeholder="请输入操作人" />
        </el-form-item>
        <el-form-item label="${comment}" prop="searchValue">
          <el-input v-model="form.searchValue" type="textarea" placeholder="请输入内容" />
        </el-form-item>
        <el-form-item label="0正常 2删除" prop="delFlag">
          <el-input v-model="form.delFlag" placeholder="请输入0正常 2删除" />
        </el-form-item>
      </el-form>
...
<script>
import { listMail, getMail, delMail, addMail, updateMail } from "@/api/bussiness/mail";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除1v1站内信编号为"' + ids + '"的数据项？').then(function() {
        return delMail(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('bussiness/mail/export', {
        ...this.queryParams
